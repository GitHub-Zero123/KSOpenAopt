...
```json
{
    "package": "test_package",  // 应修改为您的项目名(需遵循命名规范)
    "jsonOptimize": true,
    "pyReload": true,
    // "debug": false,
    // "addon++": false,
...
通用模型规范文件: `Models/global_3d_model.json`

- **面部表情**: 为了兼容剑魂，无论是否需要使用KID面部表情，都应设计面部模型。若要同时支持面部表情，可在模型上后移常态面部0.01单位以避免KID重叠闪烁
- **刀光位置**: `global_3d_model`模型中提供的定位器用于确定刀光渲染位置，不可缺失，但可以改动位置

**骨骼兼容问题**:
- **KID动作优化**: 由于历史实现问题，部分骨骼（bones）不得缺少，否则可能出现动画异常
    - 不可缺失部位: `*_covered`
    - 可以缺失部位: 双层皮肤、弯曲凹凸部位
    - 废弃部位（可选择保留/删除）: `_kid_move_root`
- **剑魂战斗优化**: 由于历史实现问题，`player`组不可缺失，用于调整复杂的旋转过渡，但不推荐使用（KID中不受支持）用于兼容剑魂内置动作
    - 可以缺失部位: 双层皮肤、弯曲凹凸部位
- **开发建议**: 直接在该模型模板上修改 不需要的bone清空模型块但保留组

### 自定义刀光纹理
在path目录(aopt/*.json)创建任意名称的json文件(允许中文)并声明其类型为 custom_sword_trail_textures
```json
{
    // 注: 为确保性能优化请控制刀光纹理在512x以内
...
## 商业化授权
使用该创作工具开发的MOD扩展项目完全开放，无任何条件限制。您可以自由使用、修改、分发和商业化本工具及其衍生作品，无需获得额外授权或支付任何费用。